...
- **Идентификаторы звуков.** Таблицу `TriggerSounds` нужно пересобрать под 26.2: между версиями
  звуки добавляются и переименовываются. Пока не проверено ни одного id — это отдельная задача,
  и до неё документация по пакам не должна утверждать, что список актуален для 26.2.
